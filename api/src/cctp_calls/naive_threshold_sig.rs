...
use primitives::{
    crh::FieldBasedHash, signature::schnorr::field_based_schnorr::FieldBasedSchnorrPk,
};
use rand::rngs::OsRng;
...
mod test {
    ...
    fn sample_calls_naive_threshold_sig_circuit() {
        ...
        create_sample_naive_threshold_sig_circuit(
            0,
            pk_path_no_bwt.to_str().unwrap(),
            vk_path_no_bwt.to_str().unwrap(),
            proof_path_no_bwt.to_str().unwrap(),
        );
