...
fn blake_test(){
    ...
    //H(a||b) = h.update(a).update(b).finalize()
    let mut a_copy = a.clone();
    a_copy.extend_from_slice(b.as_slice());

    let h = Params::new()
        .hash_length(32)
        .hash(a_copy.as_slice());

    let h_test = Params::new()
        .hash_length(32)
        .to_state()
        .update(a.as_slice())
        .update(b.as_slice())
    ...
    // H(a||b) != h.update(a).finalize().update(b).finalize()
    let mut h_test_f = Params::new()
        .hash_length(32)
        .to_state();

    h_test_f.update(a.as_slice());
    h_test_f.finalize();
    h_test_f.update(b.as_slice());
    let h_test = h_test_f.finalize();
